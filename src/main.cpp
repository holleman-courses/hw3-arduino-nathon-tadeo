...
void loop() {
  ...
  // check if characters are available from terminal input
...
void run_tflm_inference(int *int_array) {
  ...
  for (int i = 0; i < REQUIRED_INPUT_SIZE; i++) {
    // Normalize input if the model expects float values in a specific range
    input_tensor->data.f[i] = static_cast<float>(int_array[i]) / 127.0f;  
  }
  ...
  Serial.println(prediction, 6);  // Print with 6 decimal places
}
